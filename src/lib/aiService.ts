...
Coordinates: [row, col] where both are 0, 1, or 2.`,
      ...
      if (
        !aiResponse.move ||
        !Array.isArray(aiResponse.move) ||
        aiResponse.move.length !== 2
      ) {
      ...
      if (board[row][col] !== null) {
        console.warn('AI tried to move to occupied cell:', {
          row,
          col,
  ...
  private findCriticalMove(
    ...
    if (
      !board ||
      board.length !== 3 ||
      !board.every((row) => row && row.length === 3)
    ) {
    ...
    if (
      ...
      if (board?.[1]?.[1] === null) {
        return {
          move: [1, 1],
          explanation:
            'Emergency fallback move to center due to invalid board state.',
        }
      }
        ...
        for (let c = 0; c < 3; c++) {
          if (board?.[r]?.[c] === null) {
            return {
              move: [r, c],
              explanation:
                'Emergency fallback move due to invalid board state.',
            }
          }
        }
      }
